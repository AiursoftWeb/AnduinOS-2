"""Validated runtime registry for executable installed-system feature suites."""
...
class TestProfile(str, Enum):
    ...
    NIGHTLY_ONLINE = "nightly-online"
    PLATFORM_LAB = "platform-lab"
...
class FeatureSuiteRegistry:
    ...
    def select(
        ...
        if profile is TestProfile.INSTALL:
            if identifiers:
                raise ConfigurationError(
                    "Feature suites require --profile release-gate, "
                    "nightly-online, or platform-lab"
                )
            return ()
        selected = tuple(
            suite
            ...
            and profile in suite.profiles
            and (not identifiers or suite.id in identifiers)
        )
        if profile is TestProfile.PLATFORM_LAB and not selected:
            raise ConfigurationError(
                "platform-lab has no executable platform runner configured; "
                "refusing an empty test run. GNOME Boxes resize, authenticated "
                "WeChat tray, and physical Wi-Fi migration remain planned-only "
                "until a dedicated runner declares those capabilities"
            )
        if identifiers and len(selected) != len(set(identifiers)):
            unavailable = sorted(set(identifiers) - {suite.id for suite in selected})
            raise ConfigurationError(
                f"Suite(s) do not support {architecture.value}/{profile.value}: "
                + ", ".join(unavailable)
            )
        return selected
    ...
    def validate_sources(
        ...
        for suite in suites:
            ...
            if source not in selected_case_ids:
                missing.append(f"{suite.id} requires --case {source}")
        if missing:
            raise ConfigurationError(
...
def _load_suite(value: object, matrix: TestMatrix) -> FeatureSuite:
    ...
    try:
        profiles = tuple(TestProfile(item) for item in value["profiles"])
    except (TypeError, ValueError) as error:
        raise ConfigurationError(f"{identifier}: invalid profile") from error
    if not profiles or len(profiles) != len(set(profiles)):
        raise ConfigurationError(f"{identifier}: profiles must be unique and non-empty")
    if TestProfile.INSTALL in profiles:
        raise ConfigurationError(
            f"{identifier}: overlay suites cannot enter the installation profile"
        )
